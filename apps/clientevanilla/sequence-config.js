...
function validateSequence(sequence) {
  ...
  sequence.scenes?.forEach((scene, index) => {
    ...
    if (!scene.duration || scene.duration < SCENE_DURATION_LIMITS.min || scene.duration > SCENE_DURATION_LIMITS.max) {
      errors.push(`Scene ${scene.sceneId} duration must be between ${SCENE_DURATION_LIMITS.min}ms and ${SCENE_DURATION_LIMITS.max}ms`);
    }
  });
...
function getAvailableScenes() {
  return Object.keys(window.SCENES_CONFIG || SCENES_CONFIG).filter(sceneId => sceneId !== 'base');
}

/**
 * Create a custom sequence from scene list
 */
function createCustomSequence(sceneIds, baseDuration = SCENE_DURATION_LIMITS.default) {
  ...
  return {
    ...
    scenes: scenes,
    totalDuration: calculateSequenceDuration({ scenes, transitions: TRANSITION_TIMING }),
    autoLoop: false,
    showScreensAutomatically: true,
    transitions: {
...
console.log("📋 Sequence configurations loaded:", Object.keys(SEQUENCE_CONFIGS));
console.log("🎬 Total sequences available:", Object.keys(SEQUENCE_CONFIGS).length);
...
Object.entries(SEQUENCE_CONFIGS).forEach(([id, sequence]) => {
  ...
  console.log(`⏱️ ${sequence.name}: ${minutes}:${seconds.toString().padStart(2, '0')} (${sequence.scenes.length} escenas)`);
});
