...
export class AssetPreloader {
  ...
  public async preloadAll(): Promise<PreloadedAssets> {
    ...
    try {
      ...
      const realScenes = manifest.scenes.filter(
        (scene) =>
      ...
      await this.preloadAssetsWithConcurrency(
        realScenes,
        imageCache,
        audioCache
      );
  ...
  private async loadManifest(): Promise<AssetManifest> {
    try {
      ...
      throw new Error(
        `Failed to load asset manifest: ${(error as Error).message}`
      );
    }
  }
  ...
  private async preloadAssetsWithConcurrency(
    ...
    for (const scene of scenes) {
      // Check if the scene uses a video instead of an image
      const isVideo =
        scene.image360.includes(".mp4") ||
        scene.image360.includes(".webm") ||
        scene.image360.includes(".ogg");

      if (!isVideo) {
        // Preload image only if it's not a video
        promises.push(
        ...
        // Skip video preloading for now - videos are loaded dynamically in A-Frame
        logger.info("🎥 Video detectado, saltando precarga:", scene.image360);
  ...
  private incrementProgress(): void {
    ...
    this.updateProgress(
      `${this.loadedAssets}/${this.totalAssets} assets cargados`,
      percentage
    );
  }
