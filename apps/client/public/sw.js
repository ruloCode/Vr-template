...
const STATIC_ASSETS = ["/", "/manifest.webmanifest", "/asset-manifest.json"];
...
self.addEventListener("install", (event) => {
  ...
  event.waitUntil(
    caches
      ...
      .then(() => {
        // No hacer skipWaiting automáticamente, esperar a que el usuario decida
        console.log("[SW] Installation complete, waiting for user action");
      })
  );
});
...
self.addEventListener("activate", (event) => {
  ...
  event.waitUntil(
    caches
      ...
      .then(() => {
        // Notificar a todos los clientes que el nuevo SW está activo
        return self.clients.matchAll().then((clients) => {
          clients.forEach((client) => {
            client.postMessage({
              type: "SW_ACTIVATED",
              message: "Nueva versión del Service Worker activada",
            });
          });
        });
      })
